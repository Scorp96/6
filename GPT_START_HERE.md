...
The current candidate is `TEST_VERIFIED` and has a scoped `LIVE_VERIFIED` two-Worker submit/capture receipt plus one scoped live rate-limit recovery observation. The real-code Worker workload is currently `BLOCKED` before local execution; Master replacement, unattended stability, and production cutover remain unverified. Historical real-code Worker evidence belongs to older candidates and cannot be promoted automatically. This is still not `ACCEPTED`: release identity, restart recovery, symmetric rate-limit recovery, scheduled-task cutover, and production cutover remain separate gates. The production SQLite incident is recovered logically and is not hidden as an unchanged-production claim.
The current candidate has a scoped real two-Worker browser submit/capture receipt in `docs/handoffs/SCORP_V4_LIVE_CANARY_CURRENT_5BF7953.json`. It does not prove a real repository task over ChatGPT, live Master replacement, scheduled-task registration, production cutover, or long-duration unattended stability. The live observation is recorded in `docs/handoffs/SCORP_V4_RATE_LIMIT_RECOVERY_LIVE_OBSERVATION_5BF7953.json`: one real session recovered after the explicit acknowledgement without resending; the other Worker session became uncontrollable and was not retried. Do not claim the full rate-limit or real-code gates pass.
