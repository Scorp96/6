...
python -B .\scorp-agent\chatgpt-gui-bridge\tools\v4_browser_fill_diagnostic.py `
  ...
  --candidate-commit 82f0f50597ebc194f21cb383e46b52ba9a3f3b10 `
  --candidate-manifest C:\ScorpAgent\worktrees\v4-fast-runtime-command-core\docs\handoffs\SCORP_V4_FAST_RUNTIME_CANDIDATE_MANIFEST_82F0F50.json `
  --manifest-sha256 657093e3647aa926d02d7af3231cce1f8f1f6ba7c673fb53470b899e21eac07f
```
...
- Current code candidate: `82f0f50597ebc194f21cb383e46b52ba9a3f3b10` (`feat: require explicit blocked task replanning`) on `feature/v4-fast-runtime-command-core`; it adds an explicit, fenced replan path for a BLOCKED Worker result, requiring a new objective hash, current state version, current Master epoch, and a durable `TASK_REPLANNED` event before a new assignment can be created. The predecessor `5bf7953` browser and real-code evidence is historical and is not promoted to this candidate. The current candidate has no live browser canary because the existing ChatGPT tab remains rate-limited.
...
The predecessor's two-Worker browser receipt is `docs/handoffs/SCORP_V4_LIVE_CANARY_CURRENT_5BF7953.json`; it belongs to `5bf7953`, not the current `82f0f50` candidate. The current live blocker is recorded in `docs/handoffs/SCORP_V4_RATE_LIMIT_RECOVERY_LIVE_RECHECK_82F0F50_20260915.json`. Do not claim a current-candidate browser or real-code pass.
...
The subsequent read-only recheck of that same tab still showed the dialog and no composer; evidence: `docs/handoffs/SCORP_V4_RATE_LIMIT_RECOVERY_LIVE_RECHECK_82F0F50_20260915.json`. Do not create a new session or resend while this physical actor remains unresolved.
