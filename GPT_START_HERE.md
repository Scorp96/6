...
The current candidate is `TEST_VERIFIED` and has a scoped `LIVE_VERIFIED` two-Worker submit/capture receipt. The real-code Worker workload, Master replacement, unattended stability, and production cutover remain unverified. Historical real-code Worker evidence belongs to older candidates and cannot be promoted automatically. This is still not `ACCEPTED`: release identity, restart recovery, unattended stability, rate-limit popup branch, scheduled-task cutover, and production cutover remain separate gates. The production SQLite incident is recovered logically and is not hidden as an unchanged-production claim.
