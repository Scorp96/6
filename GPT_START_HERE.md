...
python -B .\scorp-agent\chatgpt-gui-bridge\tools\v4_browser_fill_diagnostic.py `
  ...
  --candidate-commit 1d55b7acab3a91bf8c6ada23c7f951f59563cd2d `
  --candidate-manifest C:\ScorpAgent\worktrees\v4-fast-runtime-command-core\docs\handoffs\SCORP_V4_FAST_RUNTIME_CANDIDATE_MANIFEST_1D55B7A.json `
...
`docs/handoffs/SCORP_V4_RUNTIME_CONNECTOR_WINDOWS_LOOP_1D55B7A.json`. This proves
...
`docs/handoffs/SCORP_V4_LIVE_CANARY_FAILURE_C20561B.json`. The newest read-only target-state receipt is `docs/handoffs/SCORP_V4_LIVE_READONLY_PREFLIGHT_1D55B7A.json`. Read-only
...
- Current code candidate: `44b5809` (`fix: detect lost worker leases in arbiter snapshot`) on `feature/v4-fast-runtime-command-core`; the five-minute wait and one-refresh boundary, fail-closed rate-limit transport errors, durable heartbeat/progress separation, and lost-worker detection are recorded in the current candidate evidence.
- Latest rate-limit failure-boundary evidence: `docs/handoffs/SCORP_V4_RATE_LIMIT_RECOVERY_FAILURE_BOUNDARY_1D55B7A.json`.
...
- Current live blocker receipt: `docs/handoffs/SCORP_V4_LIVE_READONLY_PREFLIGHT_1D55B7A.json`; prior ambiguous canary receipt: `docs/handoffs/SCORP_V4_LIVE_CANARY_FAILURE_C20561B.json`.
