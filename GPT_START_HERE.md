...
python -B .\scorp-agent\chatgpt-gui-bridge\tools\v4_browser_fill_diagnostic.py `
  ...
  --candidate-commit d4cc823e90563ae431d0050bf9313ea4d18cc088 `
  --candidate-manifest C:\ScorpAgent\worktrees\v4-fast-runtime-command-core\docs\handoffs\SCORP_V4_FAST_RUNTIME_CANDIDATE_MANIFEST_D4CC823.json `
...
`docs/handoffs/SCORP_V4_RUNTIME_CONNECTOR_WINDOWS_LOOP_D4CC823.json`. This proves
...
`docs/handoffs/SCORP_V4_LIVE_CANARY_FAILURE_C20561B.json`. The newest read-only target-state receipt is `docs/handoffs/SCORP_V4_LIVE_READONLY_PREFLIGHT_D4CC823.json`. Read-only
...
- Current code candidate: `d4cc823` (`fix: separate heartbeat from runtime progress`) on `feature/v4-fast-runtime-command-core`; the five-minute wait and one-refresh boundary, fail-closed rate-limit transport errors, and durable heartbeat/progress separation are recorded in the current candidate evidence.
- Latest rate-limit failure-boundary evidence: `docs/handoffs/SCORP_V4_RATE_LIMIT_RECOVERY_FAILURE_BOUNDARY_D4CC823.json`.
...
- Current live blocker receipt: `docs/handoffs/SCORP_V4_LIVE_READONLY_PREFLIGHT_D4CC823.json`; prior ambiguous canary receipt: `docs/handoffs/SCORP_V4_LIVE_CANARY_FAILURE_C20561B.json`.
