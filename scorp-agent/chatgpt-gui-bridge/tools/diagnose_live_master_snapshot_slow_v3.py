...
async def main():
    ...
    driver = WindowsMcpActorDriverV3(poll_page_wait_seconds=10)
    ...
    summary = {
        ...
        "has_stop_generating": any(x in snapshot for x in ("Stop generating", "Stop responding")),
        "has_retry_signal": any(x in lower for x in ("retry", "regenerate", "try again")),
        "has_error_signal": any(x in lower for x in ("something went wrong", "network error")),
        "has_message_editor": "Message ChatGPT" in snapshot,
    }
    print("LIVE_MASTER_SLOW_SNAPSHOT_DIAG=" + json.dumps(summary, sort_keys=True, separators=(",", ":")))
