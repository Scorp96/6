...
这是 `Scorp96/6` 的隔离实施分支 `feature/v4-fast-runtime-command-core`。当前代码候选为 `9aacd7c1b4b3a229c4fa61d44ac88eac95a570fa`，文档与证据提交为 `2e1dcf8` 及后续当前证据提交。实现目标是给本地 GPT/浏览器桥接一个受限的 JSON 命令边界；它不会把普通 GPT 的自然语言变成任意 PowerShell、Python、Git 或浏览器操作。
...
- LIVE_VERIFIED: Current candidate 9aacd7c1b4b3a229c4fa61d44ac88eac95a570fa was not sent through a new browser canary after the code change; the prior 83c07cc Chrome Use evidence is historical and cannot be inherited. This turn verified offline regression and candidate identity only.
