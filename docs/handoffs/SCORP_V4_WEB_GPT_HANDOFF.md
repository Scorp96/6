...
当前候选版本是 GPT-5.6 Sol 兼容路径，代码候选提交为 `12fc17b491affd5d7b1989e1d97a21e09fefeb83`。该身份与 `docs/handoffs/SCORP_V4_FAST_RUNTIME_COMMAND_CORE_VALIDATION.json`、`SCORP_V4_FAST_RUNTIME_CANDIDATE_MANIFEST_12FC17B.json` 和 `SCORP_V4_FAST_RUNTIME_PREFLIGHT_12FC17B.json` 一致。交接文件定义的是连接边界和使用顺序，不把规划文档当成运行证据。
