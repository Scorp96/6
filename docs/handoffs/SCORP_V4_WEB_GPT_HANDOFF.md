...
当前候选版本是 GPT-5.6 Sol 兼容路径，代码候选提交为 `f32e914f297893d97d78669e0e91eb4c08b3d486`。该身份与 `docs/handoffs/SCORP_V4_FAST_RUNTIME_COMMAND_CORE_VALIDATION.json`、`SCORP_V4_FAST_RUNTIME_CANDIDATE_MANIFEST_F32E914.json` 和 `docs/handoffs/SCORP_V4_LIVE_CANARY_SUCCESS_852567B.json` 一致。历史真实双 Worker 无害 Canary 已捕获两个响应且没有重复提交，但它属于旧候选；当前候选尚未重新进行真实浏览器验证。这不等于真实代码工作负载、重启恢复、无人值守稳定性或生产切换通过。交接文件定义的是连接边界和使用顺序，不把规划文档当成运行证据。
