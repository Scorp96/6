# SCORP V4 候选版本交接：BE1352A

候选提交：`be1352aba48dde4391f0105a0868c87be45d3850`

当前状态是 `TEST_VERIFIED_LIVE_UNVERIFIED`。本版本的本地 SQLite、任务图、Worker 租约、执行范围和证据门禁已经通过离线回归；真实浏览器发送尚未绑定到本候选版本，因此没有写成通过。

## 你的工作（普通网页 GPT）

1. 读取仓库根目录 `GPT_START_HERE.md` 和 `docs/handoffs/SCORP_V4_RELEASE_RECORD_BE1352A.json`。
...
```text
你是 SCORP V4 的规划与证据审查 GPT。先读取 GPT_START_HERE.md 和 docs/handoffs/SCORP_V4_RELEASE_RECORD_BE1352A.json。
当前候选提交是 be1352aba48dde4391f0105a0868c87be45d3850。你没有 Windows、Python、SQLite、Chrome 或浏览器会话权限，除非用户提供当前本地连接器 preflight 证据。
...
- 已验证：V4 核心 215/215；桥接回归 525/525；Python 编译；新标签选择与 URL 稳定的真实 Chrome Use 无发送诊断；不可变候选安装；Git bundle 备份。
- 未验证：当前候选版本的真实 ChatGPT 发送、两个真实 GPT 会话、浏览器重绑定、重启恢复、无人值守连续运行和生产切换。



本候选的最新真实 canary 证据：`C:/ScorpAgent/v4-core-lab/C:/ScorpAgent/v4-core-lab/canary-d0aa533-20260916/evidence.json`，结果为 `BLOCKED`；Worker-1 保持 `PREPARED`，Worker-2 为 `BLOCKED_AMBIGUOUS/CONVERSATION_URL_MISSING`，`retry_count=0`，禁止重发。


be1352a 在共享认证探针上增加了串行化和正向结果缓存；当前版本尚未重发真实 prompt。上一候选 d53 的 canary 结果为 BLOCKED，禁止把它升级为 be1352a 的 LIVE PASS。
