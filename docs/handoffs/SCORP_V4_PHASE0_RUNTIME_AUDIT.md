...
current documentation snapshot (code candidate `141a6450369984a765b753d091e14caec73b666d`; exact branch HEAD is recorded after this documentation update)
...
- `git diff --check`：通过。
- validation JSON：可解析。
- 当前候选 validation：代码候选 `141a6450369984a765b753d091e14caec73b666d`；请求限制 fail-closed 专项证据见 `SCORP_V4_RATE_LIMIT_RECOVERY_FAILURE_BOUNDARY_141A645.json`。
- 当前候选真实 Chrome：`LIVE_VERIFIED = BLOCKED`，原因是当前 canary 的 Chrome Use send ref 无法解析。
- Windows supervision snapshot：见 `SCORP_V4_WINDOWS_SUPERVISION_AUDIT_141A645.json`；生产旧 bridge worker 曾出现重复进程，候选进程锁仅在隔离代码中验证。
- 当前候选：`ACCEPTED = false`。
- 生产切换：未授权、未执行。
...
- 最新只读 Windows/仓库/进程/任务/SQLite 路径审计见 SCORP_V4_PHASE0_LOCAL_AUDIT_141A645.json（candidate code 141a6450369984a765b753d091e14caec73b666d；生产未修改）。该快照观察到生产 bridge worker 有两个相关进程，故生产 P0-02 仍未验证。
